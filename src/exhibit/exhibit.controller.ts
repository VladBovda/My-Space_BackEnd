...
export class ExhibitController {
  ...
  @ApiResponse({
    status: 201,
    description: 'The exhibit has been successfully created.',
    type: Exhibit,
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'List of exhibits created by the user',
    type: [Exhibit],
  })
  @UseGuards(JwtAuthGuard)
  async findMyPosts(@Request() req, @Query() query: QueryExhibitDto) {
    const { page = 1, limit = 10 } = query;
    const exhibits = await this.exhibitService.findByUserId(
      req.user.id,
      page,
      limit,
    );
  ...
  @ApiResponse({
    status: 200,
    description: 'The exhibit has been successfully deleted.',
  })
  @ApiResponse({ status: 404, description: 'Exhibit not found' })
  @UseGuards(JwtAuthGuard)
